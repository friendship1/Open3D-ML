...
class SemanticKITTISK(BaseDataset):
    """This class is used to create a dataset based on the SemanticKITTISK
    ...
    def __init__(self,
        ...
        data_config = join(dirname(abspath(__file__)), '_resources/',
                           'semantic-kitti_class8.yaml')
        DATA = yaml.safe_load(open(data_config, 'r'))
        remap_dict = DATA["learning_map_inv"]

        # make lookup table for mapping
        max_key = max(remap_dict.keys())
        remap_lut = np.zeros((max_key + 300), dtype=np.int32)
        remap_lut[list(remap_dict.keys())] = list(remap_dict.values())

        remap_dict_val = DATA["learning_map"]
        max_key = max(remap_dict_val.keys())
        remap_lut_val = np.zeros((max_key + 300), dtype=np.int32)
    ...
    def get_split(self, split):
        ...
        """
        return SemanticKITTISKSplit(self, split=split)
...
class SemanticKITTISKSplit(BaseDatasetSplit):
...
DATASET._register_module(SemanticKITTISK)
