...
class PandasetSK(BaseDataset):
    """This class is used to create a dataset based on the PandasetSK
    ...
    def get_split(self, split):
        ...
        """
        return PandasetSKSplit(self, split=split)
...
class PandasetSKSplit(BaseDatasetSplit):
    ...
    def get_data(self, idx):
        ...
        points = self.dataset.load_pc_kitti(pc_path)
        ...
        if not exists(label_path):
            ...
            labels = self.dataset.load_label_kitti(
                label_path, self.remap_lut_val).astype(np.int32)
            labels = np.fromiter(map(lambda x: self.class_map[x], labels), dtype=np.int32)
...
DATASET._register_module(PandasetSK)
